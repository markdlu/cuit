...
#this function will display the usage requirements for the script
 ...
 PARAM 3 - Action [ precheck | applywithohonly | rollback ]
 PARAM 4 - Email [UNI] 

 Usage: $PNAME [HOST_name]   [Database_Name]  [ precheck|applywithohonly|rollback] [Uni] 
    "
    exit 1
}
...
#check /orabase size, must be 8G more
 OHSIZE=`ssh $HOST "df -m /orabase |tail -1 " |awk '{print $4}'`   >> ${LOGFILE} 2>&1
 if [ "$OHSIZE" = "*'%'" ]
 then
  OHSIZE=`ssh $HOST "df -m /orabase  " |awk '{print $3}'`   >> ${LOGFILE} 2>&1
 fi
 echo "OHSIZE"  "$OHSIZE"
...
if test $# -ne 4 
 ...
 if [ $ACTION != "precheck" ] && [ $ACTION != "applywithohonly" ] && [ $ACTION != "rollback" ]
 then
   echo $ACTION
  f_usage
   return 1
...
export SQLDBA
 ...
 LOGFILE=/tmp/apply_psu_q3_2018_${ORACLE_SID}_${ACTION}.log
 echo $NOW > $LOGFILE 
